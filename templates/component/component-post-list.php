...
} else if ($spot_type === 'solarpanel') {
    $intro = get_field('solarpanel_intro_text_field', $ID);
    $image = get_field('solarpanel_featured_img_field', $ID);
}

get_template_part('templates/layout/layout', 'post-list-item', [
    // 'image' => $image,
    // 'style' => $style,
    // 'intro' => $intro,
]);
